...
def say_message(message_to_say: str):
    """
...
def get_plate_data(plate_str: str) -> Union[Dict[str, Union[str, Any]], Any]:
    ua = UserAgent()
    headers = {
        "accept": "*/*",
...
def handle_fault(
    plate_data: Union[Dict[str, Union[List[Dict[str, str]], Any]], Any]
) -> None:
...
def process_plate_queue():
    while True:
        ...
        if plate_data is not None:
            ...
            logger.warning(f"No data for {plate_number}")

        plate_queue.task_done()
        logger.info("-" * 20)


def get_match_plate(input_message: str):
...
async def on_comment(event: CommentEvent):
    ...
    if match_plate:
        ...
        plate_queue.put({
            "plate_owner": event.user.nickname,
